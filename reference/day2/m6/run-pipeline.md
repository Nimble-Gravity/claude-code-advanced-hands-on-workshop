...
and you enforce the control rules below between stages.

**Preconditions:** `artifacts/day2/m6/plan.md` must already exist (produced by
...
1. **explorer** — invoke the `explorer` subagent against the plan. It is
   read-only and writes `artifacts/day2/m6/exploration.md`.
2. **implementer** — invoke the `implementer` subagent with the plan and the
   ...
   change summary to `artifacts/day2/m6/impl-summary.md`.
...
- **Blocker → one loop → escalate.** If a stage's output contains a
  ...
  paper over it. Write a short note to `artifacts/day2/m6/escalation.md` naming
...
- **Low-confidence halt.** If any stage reports it cannot proceed with
  ...
  Record why in `artifacts/day2/m6/escalation.md`.
- Every stage's subagent invocation must pass along the plan and the prior
  ...
  point is that stages communicate through `artifacts/day2/m6/`, not through
