<!-- This orchestrator carries only a loose, implicit one-pass rule (see
     "Blocker -> one loop -> escalate" below) and no other resource bounds -
     no explicit subagent cap, no depth limit, no numeric revision-loop cap,
     no budget, no named stop conditions. Module 4 makes the implicit rule
     explicit and adds the rest. -->
---
...
- **Blocker → one loop → escalate.** If a stage's output contains a
  ...
  over it. Write a short note to `artifacts/day2/capstone/escalation.md` naming the
