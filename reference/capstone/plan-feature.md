...
to a ticket file (e.g. `scenarios/capstone-expiring-holds.md`). Treat the
...
2. **Write an intent doc** to `artifacts/capstone/intent.md`, following the
   ...
   than guessing.
3. **Write a technical plan** to `artifacts/capstone/plan.md`, following the
...
4. **Acceptance criteria must be testable statements about observable
   state** (e.g. "a post() call against a transfer whose authorization hold
   has expired raises/returns X, and does not insert a ledger entry row" —
   not "expiry works correctly"). Cross-reference each acceptance criterion
   in the plan's "Test expectations" section with what would prove it.
